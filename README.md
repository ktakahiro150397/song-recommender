...
曲調が似ている楽曲をベクトルDBから検索し、類似度の高いものを出力するアプリケーション。
...
- **🔍 曲調おすすめプレイリスト**: 指定した楽曲から似た曲を連鎖的に検索してプレイリストを作成
- **🎵 個別曲検索**: キーワードで楽曲を検索・一覧表示
- **📤 楽曲ファイルアップロード**: 複数の楽曲ファイルをサーバーにアップロード
- **🗄️ DBメンテナンス**: データベースの管理・曲の削除・楽曲登録
- **📺 YouTubeチャンネル登録**: YouTube MusicチャンネルのURL管理
- **📋 チャンネル一覧**: 登録チャンネルの確認・ダウンロードスクリプト生成
...
### 6. YouTubeからダウンロードスクリプト生成

登録済みYouTubeチャンネルの楽曲をダウンロードするPowerShellスクリプトを生成します。
...
出力スクリプトは `output_scripts/{タイムスタンプ}/` に保存されます。
...
- [x] **Phase 3.5**: Streamlit Webアプリ化（マルチページ対応）
- [x] **Phase 4**: YouTubeチャンネル管理・ダウンロードスクリプト生成
- [ ] **Phase 5**: 精度向上が必要な場合、CLAP / OpenL3 等の深層学習モデルを導入
