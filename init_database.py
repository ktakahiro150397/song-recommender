            ...
            with get_session() as session:
                for i, song_id in enumerate(song_ids):
                    ...
                    song = Song(
                        ...
                        registered_at=datetime.fromisoformat(metadata["registered_at"]) 
                            if metadata.get("registered_at") else datetime.now(),
                        excluded_from_search=metadata.get("excluded_from_search", False),
                    )
                    session.add(song)
                    
                    # ProcessedCollectionレコードを作成
                    ...
                    if (migrated + skipped) % 100 == 0:
                        print(f"  進捗: {migrated + skipped}/{len(song_ids)} 曲処理済み")
