...
from core.models import PlaylistComment
...
def test_comment_functionality():
    ...
    try:
        ...
        if comments:
            ...
            created_comment_id = latest_comment['id']
        ...
        if created_comment_id:
            print("\n=== クリーンアップ ===")
            print(f"テストコメント (ID: {created_comment_id}) を削除中...")
            with get_session() as session:
                session.execute(
                    delete(PlaylistComment).where(PlaylistComment.id == created_comment_id)
                )
            print("クリーンアップ完了")
